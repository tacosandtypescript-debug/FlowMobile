...
Versión actual: **7.6.10**. El comando de ejecución es `flow` en ambas
plataformas.
...
- En Android, los videos se guardan en `Movies/FlowMobile` para que las galerías
  los reconozcan, y los audios en `Music/FlowMobile`. Ambos siguen visibles en
  Archivos y disponibles para compartir. FlowMobile no descarga en la carpeta
  privada si falta el permiso de almacenamiento.
- Cada archivo terminado se registra en el catálogo multimedia de Android. Se
  usa Termux:API cuando está disponible y el broadcast nativo como respaldo.
...
Para encontrarlos manualmente en Android:

- videos: **Archivos › Almacenamiento interno › Movies › FlowMobile**;
- audios: **Archivos › Almacenamiento interno › Music › FlowMobile**.

Desde esas carpetas también se puede mantener pulsado un archivo y elegir
**Compartir**. FlowMobile intenta primero el selector incluido en Termux y usa
`termux-share` como respaldo solamente si Termux:API ya está instalado.
